...
# Optional status thresholds (PASS/WARN/FAIL)
ap.add_argument("--pass-reads", type=float, default=5e6, help="Min reads for PASS [default: 5e6]")
ap.add_argument("--pass-dup", type=float, default=15, help="Max dup %% for PASS [default: 15]")
ap.add_argument("--warn-reads", type=float, default=2e6, help="Min reads for WARN [default: 2e6]")
...
def main():
    ...
    def hist_cpm(density: pd.DataFrame) -> str:
        sig_candidates = [c for c in density.columns if c.lower() in ("cpm", "signal")]
        if density.empty or not sig_candidates: return "<div class='muted'>n/a</div>"
        cpm_col = sig_candidates[0]
    ...
    <h1>{SID} — Detailed Plots</h1>""")
        ...
        if not density.empty and any(c.lower() in ("cpm", "signal") for c in density.columns):
            page.write("<div class='plot-container'>")
