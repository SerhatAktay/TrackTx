#!/usr/bin/env python3
# NF_monitor — generic Nextflow pipeline monitoring
# Monitoring tool for arbitrary Nextflow and nf-core pipelines with:
...
# - Multiple interactive views (curses / Rich / oneshot)
...
def parse_args():
    ap = argparse.ArgumentParser(
        description="NF_monitor — generic Nextflow pipeline monitoring with trends & predictions",
    ...
    ap.add_argument("--trace-dir", default="", help="Directory to search recursively for Nextflow trace*.txt files")
    ...
    ap.add_argument("--preferred-logs", default="", help="Comma-separated regex patterns for preferred task log filenames")
    ap.add_argument("--stage-regex", default="", help="Regex to detect stage lines in module logs")
...
class World:
    ...
    preferred_log_patterns: Optional[List[Pattern]] = None
    stage_regex: Optional[Pattern] = None
    retain_sec: int = 900
    max_tasks: int = 2000
...
def discover_trace_file(search_roots: List[str]) -> Optional[str]:
    """Search recursively under given roots for the most recent Nextflow trace*.txt file."""
    ...
    for root in search_roots:
        ...
        for dirpath, _dirs, files in os.walk(root_abs):
            for fn in files:
                name = fn.lower()
                if not name.startswith("trace") or not name.endswith(".txt"):
                    continue
                path = os.path.join(dirpath, fn)
                try:
                    with open(path, "r", encoding="utf-8", errors="ignore") as fh:
                        header = fh.readline().strip()
                    # Heuristic: header must look like a Nextflow trace table
                    if not header or "," not in header:
                        continue
                    if not any(k in header for k in ("task_id", "hash", "process", "name")):
                        continue
...
def newest_user_log(d: str, patterns: Optional[List[Pattern]] = None) -> Optional[str]:
    """Find newest user-generated log file.

    Preference order:
      1. Any file matching user-provided patterns (if given)
      2. Newest non-wrapper log-like file (stdout/stderr/log/out/err)
    """
    try:
        best = None
        mt = -1.0

        # 1) Respect user-provided patterns first, if any
        if patterns:
            for nm in os.listdir(d):
                ...
                if not os.path.isfile(p):
                    continue
                if not any(pat.search(nm) for pat in patterns):
                    continue
                try:
        ...
        # 2) Generic heuristic: newest non-wrapper log-like file
        newest = None
        mt = -1.0
        for nm in os.listdir(d):
            if nm.startswith(".command.") or nm == ".exitcode":
...
def detect_stage_from_tail(lines: List[str], stage_re: Optional[Pattern] = None) -> Optional[str]:
    """Extract current processing stage from logs.

    Order of preference:
      1. Explicit \"Step X/Y\" lines (common in many pipelines)
      2. Lines matching a user-provided stage regex (if given)
      3. Generic INFO Step lines
      4. Last short line containing \" | \" that looks like progress
    """
    # 1) Explicit \"Step X/Y\" style
    for ln in reversed(lines or []):
        m = STEP_RE.search(ln)
        if m:
            return m.group(0).strip()

    # 2) User-provided stage regex
    ...
    # 4) Fallback: last non-empty line that looks like progress (contains |)
    for ln in reversed(lines or []):
        s = ln.strip()
...
def insight(d: str, tail_n: int, world: Optional[World] = None) -> Tuple[str, List[str], List[Tuple[str, str]]]:
    """Bundle introspection: payload, log tail, outputs"""
    patterns = None
    if world and world.preferred_log_patterns:
        patterns = world.preferred_log_patterns
    user = newest_user_log(d, patterns)
    ...
    #  - For module logs, fall back to a raw tail so we keep high-level progress lines.
    ...
    stage = detect_stage_from_tail(tl, world.stage_regex if world else None)
    if ts:
        payload = ts
    elif stage:
        payload = stage
...
def classify(world: World):
    ...
    for d in active_dirs:
        ...
        payload, tl, _ = insight(d, world.tail_n, world)
        st = detect_stage_from_tail(tl, world.stage_regex)
        if st:
            t.stage = st
...
def oneshot(w: World):
    ...
    print(f"NF_monitor  {time.strftime('%H:%M:%S')}  (run:{w.meta.run_name}  session:{w.meta.session}  exec:{w.meta.executor}  container:{container})")
...
def main():
    ...
    if a.preferred_logs:
        try:
            pats: List[Pattern] = []
            for raw in (a.preferred_logs or "").split(","):
                raw = raw.strip()
                if not raw:
                    continue
                try:
                    ...
                    continue
            if pats:
                w.preferred_log_patterns = pats
        except Exception:
            pass
    if a.stage_regex:
        try:
            w.stage_regex = re.compile(a.stage_regex)
        except re.error:
            w.stage_regex = None
    
    log = abspath(a.log) if a.log else ""
    # Trace: explicit path, or auto-detect from one or more search roots
    if a.trace:
        w.trace_path = abspath(a.trace)
    else:
        search_roots: List[str] = []
        if a.trace_dir:
            search_roots.append(a.trace_dir)
        # Common defaults: current directory and any results* directories
        try:
            search_roots.append(os.getcwd())
            for d in (os.listdir(".") or []):
                if d.startswith("results") and os.path.isdir(d):
                    search_roots.append(os.path.join(os.getcwd(), d))
        except OSError:
            pass
        w.trace_path = discover_trace_file(search_roots) if search_roots else None
